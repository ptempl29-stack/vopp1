...
## Iteration 43 (2026-08-18) — v2 changeset (Claim Summary refresh + delete resilience) — TESTING ONLY, not deployed
- Applied v2 source-only changeset (vopp-emergent-changes-20260817-v2), incremental over iteration 42. DIFF files applied: `core/claim_invoices.py`, `routers/claims.py`, `i18n.js`, `pages/ClaimBuilder.js`, `pages/Claims.js`, `pages/Invoices.js`, `pages/Notes.js` + new `tests/test_claim_delete_resilience.py`. Did NOT touch `assistant.py`/`chat.py` (AI model picker + /api/chat rate-limiting intact); `.env` untouched.
- New behaviors verified: (1) Claim Builder Claim Summary auto-refreshes on focus/visibilitychange, cross-tab invoice save (localStorage key `vpp_invoice_saved_at` + `vpp:invoice-saved` event — write-side in Invoices.js:151-152, listen-side ClaimBuilder.js:75-82), and immediately before PDF download; (2) claim-packet + packet-item + invoice deletes require window.confirm, disable-while-running with spinner, and remove the DB record/reference BEFORE best-effort storage cleanup (cleanup failure surfaced, item NOT restored).
- v2's Notes.js has better load/error guards (asArray/loading/loadError) but did NOT include the row-level 'Not recorded' fallback or the `notes-page` container relocation — RE-APPLIED both my fixes on top of v2 (list line ~375, card ~410-411, page container testid).
- Testing: 15/15 focused pytest (invoice_numbers + claim_invoice_linkage + claim_delete_resilience) pass; testing agent iteration_41.json = no critical/minor issues, frontend smoke clean. Note: /api/invoices/next-number now returns MB-0066 in preview because MB-0064/0065 were issued during testing — legacy MB-1008 correctly does not drive the sequence.
- NOT DEPLOYED — user marked "test preview only do not publish".
- Applied user-uploaded source-only changeset (vopp-emergent-changes-20260817): NEW `core/invoice_numbers.py` (first-unused MB number ≥ base, so a stray high legacy number like MB-1008 never drives the sequence) + `core/claim_invoices.py` (invoice↔claim association by patient + service/session date, both creation orders, preserving explicit links); updated `core/folder_filing.py`, routers `billing.py/claims.py/fmp.py/folders.py/forms.py/notes.py/users.py`, frontend `i18n.js/ClaimBuilder.js/Claims.js/Notes.js`. It is a superset of the prior branch (keeps provider-credential upload/auto-attach, cover amendment, minutes:60, day-files). Did NOT touch `assistant.py`/`chat.py`, so the AI model picker + /api/chat rate-limiting are intact.
- Testing: 13 changeset pytest tests + 6 added integration tests = 19/19 pass (iteration_40.json). Fixed 3 gaps found: (a) Progress Notes now show `Not recorded` fallback for empty ICD-10/CPT in BOTH card and list views (was hidden/em-dash); (b) moved `data-testid="notes-page"` from the edit-form header to the real page container (editor header now `notes-editor-header`); (c) set `invoice_seq_base=64` so next invoice = MB-0064 (logic returns first-unused ≥ base; changeset default const is 63 but the "continue at MB-0064" requirement needs base 64).
- Verified in preview: next-number=MB-0064, Notes list renders fallbacks + not blank, 19/19 tests green, frontend compiles.
- NOT DEPLOYED — user marked this "for testing only"; handoff requires explicit approval before any deploy. PROD note: after approval, set invoice_seq_base to the correct floor (e.g. last real invoice + 1) in production.
- Known follow-ups (LOW): `_auto_attach_credentials` matches provider by users.name (not unique) — consider attending_provider_id linkage.
- AI Assistant now lets staff choose the model per message: **GPT (gpt-5.4)** or **Claude Sonnet 4.6 (claude-sonnet-4-6)**, both via the Emergent universal key. Backend `routers/assistant.py` whitelists models (ALLOWED_MODELS), defaults to gpt-5.4, and safely falls back to GPT on any unknown value; the chosen model is stored on each assistant message. Frontend `pages/AIAssistant.js` has a header dropdown (persisted in localStorage) and shows a per-reply model label.
- Verified: curl (Claude reply, GPT reply, invalid→gpt fallback) + screenshot (picker + labeled reply). Progress-note summarization still uses GPT (unchanged; user asked only for the Assistant picker).

## Iteration 40b (2026-08) — Applied uploaded branch (provider credentials + cover amendment)
- Applied user-uploaded zip over the project: backend `billing.py` (invoice base default 63), `claims.py` + `fmp.py` (auto-attach provider diploma/exequatur + VA disability letter into packets; enriched invoice items; new `/api/fmp/claims/{cid}/amend-cover` cover-sheet amendment with fresh veteran e-signature for substantive field changes), `users.py` (`/api/users/{uid}/credentials` upload). Frontend `i18n.js`, `ClaimBuilder.js`, `Invoices.js`, `Messages.js`, `Team.js` (provider credential upload UI).
- KEPT current `chat.py` (strict superset with rate-limit/token-cap) and `.env` (untouched). Fixed a build-breaking bug in uploaded `Team.js` (`async` was on the outer arrow fn, not the inner handler that uses `await`).
- Verified: backend imports clean, all new endpoints registered, frontend compiles, app loads (CEO/ClaimBuilder render). NOT yet e2e-tested: credential upload/auto-attach + cover amendment flows.
...
- Optional auth: if env `CHAT_API_KEY` is set, requests must send `Authorization: Bearer <key>` or `x-api-key`; if unset, the endpoint is OPEN (recommend setting a key on production to prevent LLM-credit abuse). [2026-07-27: CHAT_API_KEY now SET in preview .env; endpoint returns 401 without it. Value stored in backend/.env — must be replicated to production env on deploy.]
...
- [2026-07-27] Added safety guards to /api/chat: per-client (IP) **rate limit** `CHAT_RATE_PER_MIN` (default 30/min → 429 with Retry-After) and a **monthly token cap** `CHAT_MONTHLY_TOKEN_CAP` (default 1,000,000 tokens → 429), tracked in Mongo `chat_usage` (per-month {tokens, requests}, rough estimate = (len(prompt)+len(reply))//4). Both configurable via env; set to 0 to disable. Verified: 31st req/min → 429, over-cap → 429, usage recorded. Needs redeploy to reach production.
