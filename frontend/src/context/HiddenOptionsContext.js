...
export function HiddenOptionsProvider({ children }) {
  ...
  useEffect(() => {
    api.get("/hidden-options").then((r) => setMap(r.data || {})).catch(() => {});
  }, []);
  ...
  const hide = async (list, value) => {
    ...
    try { const r = await api.post("/hidden-options", { list: l, value: v }); setMap(r.data || {}); } catch (e) {}
  };

  const restore = async (list, value) => {
    const l = norm(list), v = String(value);
    setMap((m) => ({ ...m, [l]: (m[l] || []).filter((x) => x !== v) }));
    try { const r = await api.post("/hidden-options/restore", { list: l, value: v }); setMap(r.data || {}); } catch (e) {}
  };
